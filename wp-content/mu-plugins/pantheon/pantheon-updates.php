...
function _pantheon_fetch_custom_upstream_info() {
	...
	$pantheon_upstream_info = get_transient( '_pantheon_upstream_info' );
	// If nothing is found, fetch it
	if ( false === $pantheon_upstream_info ) {
		...
		$pantheon_upstream_info = str_replace( '.git', '', $url_path );

		set_transient( '_pantheon_upstream_info', $pantheon_upstream_info, 1 * DAY_IN_SECONDS );
	}

	return $pantheon_upstream_info;
}
