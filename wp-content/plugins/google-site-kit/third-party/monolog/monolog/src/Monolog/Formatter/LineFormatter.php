 ...
 */
class LineFormatter extends \Google\Site_Kit_Dependencies\Monolog\Formatter\NormalizerFormatter
{
    ...
    protected function normalizeException($e)
    {
        ...
        if ($previous = $e->getPrevious()) {
            do {
                $previousText .= ', ' . \Google\Site_Kit_Dependencies\Monolog\Utils::getClass($previous) . '(code: ' . $previous->getCode() . '): ' . $previous->getMessage() . ' at ' . $previous->getFile() . ':' . $previous->getLine();
            } while ($previous = $previous->getPrevious());
        }
        $str = '[object] (' . \Google\Site_Kit_Dependencies\Monolog\Utils::getClass($e) . '(code: ' . $e->getCode() . '): ' . $e->getMessage() . ' at ' . $e->getFile() . ':' . $e->getLine() . $previousText . ')';
        if ($this->includeStacktraces) {
            $str .= "\n[stacktrace]\n" . $e->getTraceAsString() . "\n";
        }
        return $str;
