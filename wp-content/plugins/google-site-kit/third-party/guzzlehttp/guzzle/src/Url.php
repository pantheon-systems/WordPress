...
class Url
{
    ...
    {
        static $defaults = ['scheme' => null, 'host' => null, 'path' => null, 'port' => null, 'query' => null, 'user' => null, 'pass' => null, 'fragment' => null];
        ...
        if ($parts['query'] || 0 !== \strlen($parts['query'])) {
            $parts['query'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Query::fromString($parts['query']);
        }
    ...
    public static function buildUrl(array $parts)
    {
        ...
        if (!empty($parts['host'])) {
            ...
            if (isset($parts['port']) && (!isset(self::$defaultPorts[$scheme]) || $parts['port'] != self::$defaultPorts[$scheme])) {
                $url .= ':' . $parts['port'];
            }
        }
    ...
    public function setScheme($scheme)
    {
        ...
        if ($this->port && isset(self::$defaultPorts[$this->scheme]) && self::$defaultPorts[$this->scheme] == $this->port) {
            $this->port = null;
        }
        $this->scheme = \strtolower($scheme);
    }
    ...
    public function removeDotSegments()
    {
        ...
        if (\substr($this->path, 0, 1) === '/' && \substr($newPath, 0, 1) !== '/') {
            $newPath = '/' . $newPath;
        }
    ...
    public function addPath($relativePath)
    {
        if ($relativePath != '/' && \is_string($relativePath) && \strlen($relativePath) > 0) {
            ...
            if ($relativePath[0] !== '/' && \substr($this->path, -1, 1) !== '/') {
    ...
    public function getQuery()
    {
        ...
        if (!$this->query instanceof \Google\Site_Kit_Dependencies\GuzzleHttp\Query) {
            $this->query = $this->query === null ? new \Google\Site_Kit_Dependencies\GuzzleHttp\Query() : \Google\Site_Kit_Dependencies\GuzzleHttp\Query::fromString($this->query);
        }
        return $this->query;
    }
    /**
