...
class Response extends \Google\Site_Kit_Dependencies\GuzzleHttp\Message\AbstractMessage implements \Google\Site_Kit_Dependencies\GuzzleHttp\Message\ResponseInterface
{
     ...
     */
    public function __construct($statusCode, array $headers = [], \Google\Site_Kit_Dependencies\GuzzleHttp\Stream\StreamInterface $body = null, array $options = [])
    {
    ...
    {
        try {
            return \Google\Site_Kit_Dependencies\GuzzleHttp\Utils::jsonDecode((string) $this->getBody(), isset($config['object']) ? !$config['object'] : \true, 512, isset($config['big_int_strings']) ? \JSON_BIGINT_AS_STRING : 0);
        } catch (\InvalidArgumentException $e) {
            throw new \Google\Site_Kit_Dependencies\GuzzleHttp\Exception\ParseException($e->getMessage(), $this);
        }
    }
    public function xml(array $config = [])
    {
        ...
        try {
            ...
            $xml = new \SimpleXMLElement((string) $this->getBody() ?: '<root />', isset($config['libxml_options']) ? $config['libxml_options'] : \LIBXML_NONET, \false, isset($config['ns']) ? $config['ns'] : '', isset($config['ns_is_prefix']) ? $config['ns_is_prefix'] : \false);
            \libxml_disable_entity_loader($disableEntities);
            \libxml_use_internal_errors($internalErrors);
        } catch (\Exception $e) {
            \libxml_disable_entity_loader($disableEntities);
            \libxml_use_internal_errors($internalErrors);
            throw new \Google\Site_Kit_Dependencies\GuzzleHttp\Exception\XmlParseException('Unable to parse response body into XML: ' . $e->getMessage(), $this, $e, \libxml_get_last_error() ?: null);
        }
        return $xml;
    }
