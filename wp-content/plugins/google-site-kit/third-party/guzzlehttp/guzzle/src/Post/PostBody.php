 ...
 */
class PostBody implements \Google\Site_Kit_Dependencies\GuzzleHttp\Post\PostBodyInterface
{
    /** @var StreamInterface */
    private $body;
    /** @var callable */
     ...
     */
    public function applyRequestHeaders(\Google\Site_Kit_Dependencies\GuzzleHttp\Message\RequestInterface $request)
    {
    ...
    public function getFields($asString = \false)
    {
        ...
        $query = new \Google\Site_Kit_Dependencies\GuzzleHttp\Query($this->fields);
        $query->setEncodingType(\Google\Site_Kit_Dependencies\GuzzleHttp\Query::RFC1738);
    ...
    public function addFile(\Google\Site_Kit_Dependencies\GuzzleHttp\Post\PostFileInterface $file)
    {
        $this->files[] = $file;
        $this->mutate();
    }
    ...
    private function getBody()
    {
        if ($this->body) {
            ...
            return $this->body = \Google\Site_Kit_Dependencies\GuzzleHttp\Stream\Stream::factory();
        }
    }
    /**
    ...
    {
        if (!$this->aggregator) {
            $this->aggregator = \Google\Site_Kit_Dependencies\GuzzleHttp\Query::phpAggregator();
        }
    ...
    private function createMultipart()
    {
        ...
        return new \Google\Site_Kit_Dependencies\GuzzleHttp\Post\MultipartBody(\call_user_func($this->getAggregator(), $this->fields), $this->files);
    }
    /**
    ...
    {
        return \Google\Site_Kit_Dependencies\GuzzleHttp\Stream\Stream::factory($this->getFields(\true));
    }
    /**
