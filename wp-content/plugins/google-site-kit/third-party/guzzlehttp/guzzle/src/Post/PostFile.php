 ...
 * Post file upload
 */
class PostFile implements \Google\Site_Kit_Dependencies\GuzzleHttp\Post\PostFileInterface
{
    private $name;
    private $filename;
    private $content;
    ...
    private function prepareContent($content)
    {
        ...
        if (!$this->content instanceof \Google\Site_Kit_Dependencies\GuzzleHttp\Stream\StreamInterface) {
            $this->content = \Google\Site_Kit_Dependencies\GuzzleHttp\Stream\Stream::factory($this->content);
        } elseif ($this->content instanceof \Google\Site_Kit_Dependencies\GuzzleHttp\Post\MultipartBody) {
    ...
    private function prepareDefaultHeaders()
    {
        ...
        if (!$this->hasHeader('Content-Type')) {
            $this->headers['Content-Type'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Mimetypes::getInstance()->fromFilename($this->filename) ?: 'text/plain';
        }
    }
    /**
