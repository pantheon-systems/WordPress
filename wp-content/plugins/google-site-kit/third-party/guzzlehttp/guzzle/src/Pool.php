 ...
 * performance.
 */
class Pool implements \Google\Site_Kit_Dependencies\GuzzleHttp\Ring\Future\FutureInterface
{
     ...
     */
    public function __construct(\Google\Site_Kit_Dependencies\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
        $this->client = $client;
        $this->iter = $this->coerceIterable($requests);
        $this->deferred = new \Google\Site_Kit_Dependencies\React\Promise\Deferred();
     ...
     */
    public static function batch(\Google\Site_Kit_Dependencies\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
        ...
        (new self($client, $requests, \Google\Site_Kit_Dependencies\GuzzleHttp\Event\RequestEvents::convertEventArray($options, ['end'], ['priority' => \Google\Site_Kit_Dependencies\GuzzleHttp\Event\RequestEvents::LATE, 'fn' => function (\Google\Site_Kit_Dependencies\GuzzleHttp\Event\EndEvent $e) use($hash) {
            $hash[$e->getRequest()] = $e->getException() ? $e->getException() : $e->getResponse();
        }])))->wait();
        return new \Google\Site_Kit_Dependencies\GuzzleHttp\BatchResults($hash);
    }
    /**
     ...
     *                                  {@see GuzzleHttp\Pool::__construct}
     */
    public static function send(\Google\Site_Kit_Dependencies\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
    ...
    {
        return \is_callable($this->poolSize) ? \call_user_func($this->poolSize, \count($this->waitQueue)) : $this->poolSize;
    }
    /**
