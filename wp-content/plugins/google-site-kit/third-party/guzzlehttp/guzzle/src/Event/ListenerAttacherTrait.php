...
trait ListenerAttacherTrait
{
     ...
     */
    private function attachListeners(\Google\Site_Kit_Dependencies\GuzzleHttp\Event\HasEmitterInterface $object, array $listeners)
    {
    ...
    private function buildListener($name, $data, &$listeners)
    {
        ...
        if (isset($data['fn'])) {
            ...
            throw new \InvalidArgumentException('Each event listener must be a ' . 'callable or an associative array containing a "fn" key.');
        }
    }
}
