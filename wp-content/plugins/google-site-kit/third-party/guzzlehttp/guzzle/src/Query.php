 ...
 */
class Query extends \Google\Site_Kit_Dependencies\GuzzleHttp\Collection
{
     ...
     * @return self
     */
    public static function fromString($query, $urlEncoding = \true)
    {
        static $qp;
        if (!$qp) {
            $qp = new \Google\Site_Kit_Dependencies\GuzzleHttp\QueryParser();
        }
        $q = new static();
        if ($urlEncoding !== \true) {
     ...
     * @return callable
     */
    public static function phpAggregator($numericIndices = \true)
    {
        return function (array $data) use($numericIndices) {
            return self::walkQuery($data, '', function ($key, $prefix) use($numericIndices) {
