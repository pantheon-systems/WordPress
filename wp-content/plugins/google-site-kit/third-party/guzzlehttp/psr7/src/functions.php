...
{
    if ($message instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface) {
        $msg = \trim($message->getMethod() . ' ' . $message->getRequestTarget()) . ' HTTP/' . $message->getProtocolVersion();
        if (!$message->hasHeader('host')) {
            $msg .= "\r\nHost: " . $message->getUri()->getHost();
        }
    } elseif ($message instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface) {
...
function stream_for($resource = '', array $options = [])
{
    if (\is_scalar($resource)) {
        ...
        return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Stream($stream, $options);
    }
    switch (\gettype($resource)) {
        case 'resource':
            return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Stream($resource, $options);
        case 'object':
            if ($resource instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\StreamInterface) {
                return $resource;
            } elseif ($resource instanceof \Iterator) {
                return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\PumpStream(function () use($resource) {
                    if (!$resource->valid()) {
                        return \false;
                    }
            ...
            break;
        case 'NULL':
            return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'), $options);
    }
    if (\is_callable($resource)) {
        return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\PumpStream($resource, $options);
    }
...
function modify_request(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $changes)
{
    ...
    if ($request instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\ServerRequestInterface) {
        return (new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\ServerRequest(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->getProtocolVersion(), $request->getServerParams()))->withParsedBody($request->getParsedBody())->withQueryParams($request->getQueryParams())->withCookieParams($request->getCookieParams())->withUploadedFiles($request->getUploadedFiles());
    }
    return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Request(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->getProtocolVersion());
}
/**
...
function parse_request($message)
{
    ...
    $request = new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? _parse_request_uri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
...
function parse_response($message)
{
    ...
    $parts = \explode(' ', $data['start-line'], 3);
    return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Response($parts[1], $data['headers'], $data['body'], \explode('/', $parts[0])[1], isset($parts[2]) ? $parts[2] : null);
}
/**
