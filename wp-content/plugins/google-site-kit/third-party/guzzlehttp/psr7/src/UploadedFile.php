...
class UploadedFile implements \Google\Site_Kit_Dependencies\Psr\Http\Message\UploadedFileInterface
{
    ...
    private function setStreamOrFile($streamOrFile)
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Stream($streamOrFile);
        } elseif ($streamOrFile instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\StreamInterface) {
    ...
    private function setError($error)
    {
        ...
        if (\false === \in_array($error, \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\UploadedFile::$errors)) {
    ...
    {
        $this->validateActive();
        if ($this->stream instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\StreamInterface) {
            return $this->stream;
        }
        return new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\LazyOpenStream($this->file, 'r+');
    }
    /**
     * {@inheritdoc}
     *
    ...
    public function moveTo($targetPath)
    {
        ...
        if ($this->file) {
            ...
            copy_to_stream($this->getStream(), new \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\LazyOpenStream($targetPath, 'w'));
            $this->moved = \true;
        }
        if (\false === $this->moved) {
