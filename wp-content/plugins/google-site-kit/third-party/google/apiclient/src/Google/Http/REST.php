...
class Google_Http_REST
{
    ...
    {
        $runner = new \Google\Site_Kit_Dependencies\Google_Task_Runner($config, \sprintf('%s %s', $request->getMethod(), (string) $request->getUri()), array(\get_class(), 'doExecute'), array($client, $request, $expectedClass));
        if (null !== $retryMap) {
    ...
    public static function decodeHttpResponse(\Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response, \Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request = null, $expectedClass = null)
    {
        ...
        if (\intval($code) >= 400) {
            ...
            throw new \Google\Site_Kit_Dependencies\Google_Service_Exception($body, $code, null, self::getResponseErrors($body));
        }
