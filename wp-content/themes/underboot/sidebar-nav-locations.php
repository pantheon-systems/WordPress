 ...
 */
 ?>
                	<ul class="menu" >						
                           <li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/" title="Home page for 360&deg; Painting">Home</a></li>
                       
 
                         <?php
						 
						 $mylocationpath = do_shortcode( '[bng_location id="location_path"]' );
							...
							if($aboutnav !== '') {	 
                                //get the URL of the about page 	
                                    $aboutnav = site_url ('/'.$mylocationpath.'/'.$aboutnav);  //default does not have a locationpath, thats why you see double // here. fix?
                                ...
                                //check if ID has children
									...
									if (empty($aboutdescendants)) {	?>			
										<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="button_1_url"]' );?>/" title="About">About 360&deg;</a></li>			
									<?php }else 
									//if has children
									{	
											$aboutincl = "";	
						...
						<?php
							...
							if($residentialnav !== '') {	 
                                //get the URL of the residential page 	
                                    $residentialnav = site_url ('/'.$mylocationpath.'/'.$residentialnav);
                                ...
                                //check if ID has children
									...
									if (empty($residentialdescendants)) {	?>			
										<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="customvar3"]' );?>/" title="Residential painting and fence and deck staining">Residential</a></li>			
									<?php }else 
									//if has children
									{	
											$residentialincl = "";	
                        ...
                        <?php
							...
							if($commercialnav !== '') {	 
                                //get the URL of the commercial page 	
                                    $commercialnav = site_url ('/'.$mylocationpath.'/'.$commercialnav);
                                ...
                                //check if ID has children
									...
									if (empty($commercialdescendants)) {	?>			
										<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="customvar4"]' );?>/" title="Commercial painting and fence and deck staining">Commercial</a></li>			
									<?php }else 
									//if has children
									{	
											$commercialincl = "";	
						 ...
						 <?php
							...
							if($serviceareanav !== '') {	 
                                //get the URL of the Service Areas page 	
                                    $serviceareanav = site_url ('/'.$mylocationpath.'/'.$serviceareanav);
                                ...
                                //check if ID has children
									...
									if (empty($serviceareadescendants)) {	?>			
										<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="customvar5"]' );?>/" title="Service areas for painting services">Service Areas</a></li>			
									<?php }else 
									//if has children
									{	
											$serviceareaincl = "";	
                        ...
                        <?php $blognav = do_shortcode('[bng_location id="customvar2"]');
                            if($blognav !== '') {?>
                          	<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="customvar2"]' );?>/" title="Blog at 360° Painting">Blog</a> </li> 
                       <?php }
                        ?>    
                        
            			 <?php $contactnav = do_shortcode('[bng_location id="button_5_url"]');
                            if($contactnav !== '') {?>
                          	<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="button_5_url"]' );?>/" title="Contact Us">Contact Us</a></li>
                       <?php } ?>	
                       ...
                       <?php
							...
							if($medianav !== '') {	 
                                //get the URL of the media page 	
                                    $mediaurl = site_url ('/'.$mylocationpath.'/'.$medianav);
                                //get the ID of the media page
                                    $mediapostid = url_to_postid( $medianav );                                
                                ?>
                                                       
                                <li style="padding-left:50px;"><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="button_3_url"]' );?>/" title="Media Room" style="color:#cccccc;">Media Room</a></li>            		
                        ...
                        <?php
							...
							if($servicesnav !== '') {	 
                                //get the URL of the services page 	
                                    $servicesurl = site_url ('/'.$mylocationpath.'/'.$servicesnav);
                                ...
                                //check if ID has children
									...
									if (empty($servicesdescendants)) {	?>			
										<li><a href="<?php echo get_site_url(); ?>/<?php echo do_shortcode( '[bng_location id="location_path"]' );?>/<?php echo do_shortcode( '[bng_location id="button_2_url"]' );?>/" title="Services">Services</a></li>			
									<?php }else 
									//if has children
									{	
											$servicesincl = "";	
